...
class BillDomain{
    ...
    public function get_pay_type_list(){
        return $this->provider->get_pay_type_list();
    }
    ...
    public function pay_check($user_id, $user_name, $money, $order_id){
        require_cache(APP_PATH.'/Common/Function/alipay.php');
        $date = date_f();
        if(!alipay_callback()){
            ...
            switch ($data['type']){
                case 1 :
                    //账户充值
                    $result = $this->recharge($user_id, $user_name, $money, $data['payment']);
                    if(is_zerror($result)){
                        $this->provider->rollback();                            //事务回滚
                        return $result;
                    }
                ...
                case 2 :
                    ...
                    $result = $domain->change_package($user_id, $data['sup']);
                    if(is_zerror($result)){
                        $this->provider->rollback();                            //事务回滚
                        return $result;
                    }
    ...
    /**
     * 充值
     * @param  <BillDomainModel> $model 账单模型
     * @return <bool> 是否成功
     */
    public function recharge($user_id, $user_name, $money, $pay_type){
        ...
        //添加充值记录
        if(!$this->provider->add_recharge_record($user_id, $user_name, $money, $date)){
            $this->provider->rollback();                    //事务回滚
            return E(ErrorMessage::$OPERATION_FAILED);      //操作失败
        }
        //更新收入
