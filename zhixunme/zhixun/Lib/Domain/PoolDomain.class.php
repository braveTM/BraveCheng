...
class PoolDomain {
    ...
    public function get_pool_list($user_id,$type,$like,$page,$size,$order) {
        $data = $this->provider->get_pool_list($user_id, intval($type), $like, $page, $size, $order);
        if($data === false)
            return E(ErrorMessage::$OPERATION_FAILED);
        return $data;
    }
    ...
    public function get_pool_total_count($user_id,$type,$like){
        $data = $this->provider->get_pool_total_count($user_id, $type, $like);
        if($data === false)
            return E(ErrorMessage::$OPERATION_FAILED);
        return $data;
    }
