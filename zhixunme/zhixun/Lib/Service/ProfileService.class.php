...
class ProfileService{
     ...
     * @param  <int>    $user_name    昵称
     * @param  <string> $contact      联系方式
     * @param  <string> $qq           QQ
     * @param  <string> $qq           用户头像
     ...
     * @param  <string> $experience   经历
     * @return <bool> 是否成功
     */
    public function update_profile($user_id, $user_name, $contact, $qq, $photo, $user_type, $gender, $province, $city, $date, $introduction, $experience){
        $model = new ProfileDomainModel();
        $model->__set('user_id', intval($user_id));
        $model->__set('user_name', $user_name);
        $model->__set('contact', $contact);
        $model->__set('qq', $qq);
        $model->__set('photo', $photo);
        $model->__set('user_type', $user_type);
        $model->__set('gender', $gender);
        $model->__set('province', $province);
        $model->__set('city', $city);
        $model->__set('date', $date);
        $model->__set('introduction', $introduction);
        $model->__set('experience', $experience);
    ...
    public function get_resource_profile_list($label, $role_id, $auth_code, $province, $city, $page, $size, $key = null, $order = null, $count = false){
        ...
        $size  = intval($size);
        if($label == null)
            $data = $this->provider->get_resource_list(intval($role_id), $auth_code, $province, $city, $page, $size, $key, $order, $count);
        else
            $data = $this->provider->get_resource_list_by_label($label, $auth_code, $province, $city, $page, $size, $key, $order, $count);
        return $data;
    }
     ...
     * @return <array> 省份列表
     */
    public function get_province_list($str){
        $provider = new ProvinceProvider();
        if (empty($str)){
            ...
            foreach($array as $key=>$value){
                $provinces[$key]=$provider->get_province($value);
            }
            return $provinces;
        }
    }
