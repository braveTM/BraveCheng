...
class ArticleService{
    ...
    public function get_article_list($class_id, $page, $size, $user_id = null, $from = null, $to = null, $status = null, $order = null, $count = false, $like = null){
        ...
        return $this->provider->get_article_list($class_id, $page, $size, $user_id, $from, $to, $status, $order, $count, $like);
    }
    ...
    public function get_fixed_article($id){
        return $this->provider->get_fixed_article($id);
    }
     ...
     * @return bool 
     */
    public function add_read_count($id){
        if(!$this->provider->increase_read_count($id)){
            return E();
        }
        return true;
    }
    ...
    public function add_praise($id){
        if(!$this->provider->increase_praise($id)){
            return E();
        }
        return true;
    }
    ...
    protected function filter_number($num){
     ...
     public function getNextPreArt($art_id){
     	$artProvider=new ArticleProvider();
     	$preArt = $artProvider->getPreArt($art_id);
     	$nextArt = $artProvider->getNextArt($art_id);
     	$preNextArt['preArt'] = $preArt;
     	$preNextArt['nextArt'] = $nextArt;
     	return $preNextArt;
     }
}
?>
