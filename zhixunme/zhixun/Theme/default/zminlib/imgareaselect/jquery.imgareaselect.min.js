...
//avatar_r_1.0.js
var avatarRender={b:function(c){try{var a=$(c).val();if(avatarRender.f(a)){$("#form_upload").submit()}}catch(b){avatarRender.g(b)}},c:function(d){$("#up_avatar_left").show();$("#n_avatar_div").empty();$("#b_avatar_div").empty();$("#m_avatar_div").empty();$("#s_avatar_div").empty();$("input[name=photo]").val(d.url);var c=document.createElement("IMG");c.id="my_avatar";window.a_width=d.width;window.a_height=d.height;if(d.width>d.height){if(d.width>200){c.style.width="200px"}}else{if(d.height>200){c.style.height="200px"}}c.src=d.url+"?"+Math.random();$("img.b_avatar").attr("src",d.url);$("#n_avatar_div").append(c);var g=$("img#my_avatar").css("width");var b=$("img#my_avatar").css("height");var f=document.createElement("IMG");f.className="b_avatar";f.src=d.url+"?"+Math.random();$("#b_avatar_div").append(f);var e=document.createElement("IMG");e.className="m_avatar";e.src=d.url+"?"+Math.random();$("#m_avatar_div").append(e);var a=document.createElement("IMG");a.className="s_avatar";a.src=d.url+"?"+Math.random();$("#s_avatar_div").append(a);setTimeout(function(){$("#my_avatar").imgAreaSelect({aspectRatio:"1:1",x1:0,y1:0,x2:100,y2:100,handles:true,enable:true,parent:$(".select_area"),selectionOpacity:0.2,imageWidth:g,imageHeight:b,fadeSpeed:200,onInit:function(){},onSelectChange:avatarRender.d});setTimeout(function(){avatarRender.d($("#n_avatar_div img")[0],{height:100,width:100,x1:0,x2:100,y1:0,y2:100})},50)},10);$("#upload_status").css("display","none");$("p.prv_see").removeClass("hidden").addClass("showp");$(".p_cont_b").css("marginLeft","40px");$(".p_cont_m").css("marginLeft","20px")},d:function(a,b){if(!b.width||!b.height){return false}avatarRender.e(a,b,100,"#b_avatar_div img");avatarRender.e(a,b,60,"#m_avatar_div img");$("#x").val(b.x1);$("#y").val(b.y1);$("#w").val(b.width);$("#h").val(b.height);return true},e:function(b,g,k,d){var a=parseInt($("#my_avatar").width(),10);var f=parseInt($("#my_avatar").height(),10);var j=k/((g.width)||1);var h=k/((g.height)||1);var i=(j*a);var c=(h*f);$(d).css({width:Math.round(i),height:Math.round(c),marginLeft:-Math.round((j*g.x1)),marginTop:-Math.round((h*g.y1))})},f:function(a){if(/^.*?\.(gif|png|jpg|jpeg|bmp)$/i.test(a)){return true}alert("图片格式不正确，请选择图片。");return false},g:function(a){if(typeof a=="undefined"){alert("上传失败，刷新页面或稍后再试。")}else{alert(a)}}};