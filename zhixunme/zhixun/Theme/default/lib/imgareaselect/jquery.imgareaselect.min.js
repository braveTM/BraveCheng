...
//    },
    ...
    c:function(img){
        ...
        temp.src=img.url+'?'+Math.random();
        $('img.b_avatar').attr('src',img.url);
        $('#n_avatar_div').append(temp);
        var lw=$('img#my_avatar').css('width');
        var lh=$('img#my_avatar').css('height');
        //big
        var b_img = document.createElement('IMG');
        b_img.className='b_avatar';
        b_img.src = img.url+'?'+Math.random();
        $('#b_avatar_div').append(b_img);
        //middle
        var m_img = document.createElement('IMG');
        m_img.className='m_avatar';
        m_img.src = img.url+'?'+Math.random();
        $('#m_avatar_div').append(m_img);
        //small
        var s_img = document.createElement('IMG');
        s_img.className = 's_avatar';
        s_img.src = img.url+'?'+Math.random();
        $('#s_avatar_div').append(s_img);
        setTimeout(function(){
        ...
        $("p.prv_see").removeClass("hidden").addClass("showp");
        $(".p_cont_b").css('marginLeft','40px');
        $(".p_cont_m").css('marginLeft','20px');
    },
    /**
     * 选择图片区域时回调函数
     * @author yoyiorlee
    ...
    d:function(img, selection){
        if (!selection.width || !selection.height)
            return false;
        avatarRender.e(img, selection,100,'#b_avatar_div img');
        avatarRender.e(img, selection,60,'#m_avatar_div img');
        $('#x').val(selection.x1);
        $('#y').val(selection.y1);
     ...
     * @date 2012-12-07
     */
    e:function(img, selection,size,e){
