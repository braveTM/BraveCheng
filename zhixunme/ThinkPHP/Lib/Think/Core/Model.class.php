...
class Model extends Think
{
    ...
    public function save($data='',$options=array()) {
        ...
        //-----------------REMOVE CACHE-----------------
        require_cache(APP_PATH.'/Common/Class/CacheConfig.class.php');
        $co = new CacheOperate();
        $co->remove_cache($options['table'], $options['where']);
     ...
     */
     public function find($options=array()) {
        ...
        require_cache(APP_PATH.'/Common/Class/CacheConfig.class.php');
        $co = new CacheOperate();
        $resultSet = $co->get_cache($options['table'], $options['where'], $cache_key);
        if(empty($resultSet)){
            $resultSet = $this->db->select($options);
            if(!empty($resultSet) && !empty($cache_key))
                $co->set_cache($options['table'], $cache_key, $resultSet);
        }
    ...
    public function setModelName($name, $pre = ''){
        $this->tablePrefix = $pre == '' ? $this->tablePrefix : $pre;
        $this->name = $name;
        $this->tableName = $name;
        $this->trueTableName = $this->tablePrefix.$this->name;
    }
     ...
     * @return <int>
     */
    public function get_trans_time(){
        return $this->db->get_trans_time();
    }
};
?>