...
function __autoload($name) {
    ...
    if(substr($name, -6) == "_model"){
        ...
        require_cache(LIB_PATH.'Model/ViewModel/'.$array[0].'/'.$array[1].'/'.$name.'.class.php');
    }elseif(substr($name, -5) == "_page"){
        require_cache(LIB_PATH.'Page/'.$name.'.class.php');
    }elseif (substr($name, -5) == "Model") {
        if (substr($name, -11) == "DomainModel") {
            require_cache(LIB_PATH . 'Model/DomainModel/' . $name . '.class.php');
        } elseif(substr($name, -9) == "ViewModel") {
            require_cache(LIB_PATH . 'Model/ViewModel/' . $name . '.class.php');
        } else {
            require_cache(LIB_PATH . 'Model/SystemModel/' . $name . '.class.php');
        }
    } elseif (substr($name, -3) == "Map") {
        require_cache(LIB_PATH . 'Map/' . $name . '.class.php');
    } elseif (substr($name, 0, 5) == "Model") {
        require_cache(LIB_PATH . 'Model/' . $name . '.class.php');
    } elseif (substr($name, -10) == "CrmService") {
        require_cache(LIB_PATH . 'Module/CRM/Service/' . $name . '.class.php');
    } elseif (substr($name, -11) == "CrmProvider") {
        require_cache(LIB_PATH . 'Module/CRM/Provider/' . $name . '.class.php');
    } elseif (substr($name, -7) == "Service") {
        require_cache(LIB_PATH . 'Service/' . $name . '.class.php');
    } elseif (substr($name, -8) == "Provider") {
        require_cache(LIB_PATH . 'Provider/' . $name . '.class.php');
    } elseif (substr($name, -6) == "Domain") {
        require_cache(LIB_PATH . 'Domain/' . $name . '.class.php');
    } elseif (substr($name, -6) == "Action") {
        require_cache(LIB_PATH . 'Action/' . $name . '.class.php');
    } elseif (substr($name, -5) == "Cache") {
        require_cache(LIB_PATH . 'Cache/' . $name . '.class.php');
    } else {
        // 根据自动加载路径设置进行尝试搜索
...
function CC($name) {
    ...
    if (is_string($name)) {
        if (!strpos($name, '.')) {
            ...
            return isset($_config_cache[$name]) ? $_config_cache[$name] : null;
        }
        // 二维数组设置和获取支持
        $name = explode('.', $name);
        $name[0] = strtolower($name[0]);
        return isset($_config_cache[$name[0]][$name[1]]) ? $_config_cache[$name[0]][$name[1]] : null;
    }
    // 批量设置
    if (is_array($name))
        return $_config_cache = array_merge($_config_cache, array_change_key_case($name));
    return null; // 避免非法参数
}
